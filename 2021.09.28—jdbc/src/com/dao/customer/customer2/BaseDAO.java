package com.dao.customer.customer2;
...
public abstract class BaseDAO<T> { //把父类定义为泛型，子类具体实现
    ...
    //获取BaseDAO的子类继承父类的泛型
    {
        ...
        Type genericSuperclass1 = genericSuperclass;
        ParameterizedType paramType= (ParameterizedType) genericSuperclass1;//类型强转
        Type[] actualTypeArguments = paramType.getActualTypeArguments();//获取父类的泛型参数
        clazz= (Class<T>) actualTypeArguments[0];//获取泛型的第一个参数 拿到customer
    }
    ...
    //通用查询返回一条记录
    public  T AnyTableSelect(Connection con,String sql, Object... args) {
        PreparedStatement ps = null;
    ...
    //通用查询返回多条数据  (考虑上事务)
    public List<T> getListSelect(Connection con,String sql, Object... args) {
        PreparedStatement ps = null;
        ResultSet resultSet = null;
        try {
            ps = con.prepareStatement(sql);
